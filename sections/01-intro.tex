...
is the \emph{excess reconstruction cost} they impose on future queries against
...
resource ephemerality, identifier loss---whose superposition produces step-shaped,
not compound, growth (\cref{sec:interest}); and past a knowable boundary the history
becomes not expensive but \emph{irrecoverable}, a mandatory write-off with no
financial analogue (\cref{sec:model-anti}).

The central claim of the paper is stated by the commissioning question and treated as
a hypothesis, not an assumption: \emph{organizations can defer evidence capture, but
...
reconstruction collapses, and---more insidiously---that heuristic reconstruction
substitutes \emph{plausible but false} history for missing history at rates that
reach 48\% of accepted links in our dense-activity condition. Deferred capture is
thus repaid in one of three currencies: effort, error, or loss.
...
\item[RQ3:] How does evidence debt influence later reconstruction effort?
...
reconstruction cost over a query workload; reconstruction as record-linkage
...
evidence-debt ``interest'' is stochastic and step-shaped rather than compound, that
...
classes degraded at 1--40\%; a pairwise arm that gives the model's central
interaction prediction a genuine chance to fail; removal and sensitivity arms
...
tests; twenty seeds; all code and data released, with results tables generated
programmatically from run outputs---including an end-to-end computation of
$\ED(t)$ itself under a declared workload (\cref{sec:simulation}). The study
...
census of 200 merged Argo CD pull requests and 30 releases under a narrow,
machine-testable repository schema. It establishes that an intent-link deficit
occurs outside the simulation, while negative controls preserve the distinction
between deficit classes; it does not measure debt or cost (\cref{sec:field}).
...
project is the bounded public-repository census of \cref{sec:field}; it is
