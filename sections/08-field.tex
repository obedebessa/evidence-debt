...
are included with the accompanying artifact.

Descriptive strata are also deterministic. Authorship is bot-authored when GitHub
types the author as \texttt{Bot}, the login ends in \texttt{[bot]}, or it matches a
Dependabot/Renovate-style login; all other authors are human-authored. Change type
is assigned in precedence order from the title prefix, labels, and at most 20
changed paths: documentation (documentation prefix/label or documentation-only
paths), dependency/maintenance (bot, dependency label/title, or maintenance
prefix), feature/bug (\texttt{feat}/\texttt{fix}/\texttt{bug} prefix or matching
label), then other. These are descriptive controls, not judgments of change value.
...
\input{empirical/argocd_validation_macros.tex}
...
it does not validate issue linkage as a measure of intent quality. A stronger
validation should add a second independent reviewer, shuffle the sample, blind both
reviewers to collector output, and report agreement (or a complete disagreement
matrix). Those steps were not performed here and no inter-rater statistic is
claimed.
...
coding, and generated counts are included in \texttt{empirical/}.
...
Timed, protocolized exercises measuring reconstruction performance (ERC and FRR;
\cref{met:erc}): trained
...
and---where independent ground truth exists---false reconstruction at the same
declared confidence/acceptance policy.
The primary excess-burden design adds an explicit reference arm. Investigators
select chains initially satisfying $\Sch$, seal independent ground truth, and
construct (i)~a complete reference view $E^{*}$ and (ii)~a treatment view $E$ by
controlled holdout of the evidence class under study. Participants and query order
are randomized and counterbalanced across matched chains to limit learning; both
views use the same preregistered $\rho$. Their effort, accepted-error, and abstention
differences estimate $\ED_\rho$, not $\ED^{\star}$. Where controlled holdout is not
permitted, a matched comparison between schema-complete and naturally deficient
changes is the weaker observational alternative and must report residual imbalance.
Every abstention is coded as a policy outcome; only cases that complete the separate
$(U,\gamma,\rho_{\mathrm{exh}})$ protocol are additionally counted in IRR. This
prevents a cautious participant or tool from creating apparent physical loss.
...
EDR and measured ERC/FRR that a validated index (\cref{met:edi}) requires.
...
The framework fails empirically if: census EDR does not predict drill ERC/FRR/AR beyond
...
schedules (against IH1 and \cref{prop:steps}); or the preregistered $E/E^{*}$
contrast shows no excess effort, accepted error, or abstention under deficit. Each
