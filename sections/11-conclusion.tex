...
which debt is expected excess effort, accepted-error, and policy-abstention burden,
with certified irrecoverability kept as a separate physical state; reconstruction is linkage
inference, and decay is a survival process; analytic results locating exactly where
the financial metaphor breaks (stochastic plateau-and-cliff effort and
abstention exposure under sound exhaustive policies, discriminator-dependent error, and a
mandatory physical write-off boundary) and
what remains when it is removed; a metric suite honest enough to reject one of its
initially proposed metrics; an executed, deterministically reproducible degradation experiment
showing that redundancy absorbs some deficits, that paired path losses expose
complementary-degradation interaction, that effort inflates before failure appears, and
that reconstruction under heavy deficit fabricates confident false history at rates
large enough to materially distort an after-the-fact operational timeline; a bounded,
...
simulation's closed world in a specific, mechanistic way (payment in effort,
accepted error, and abstention, with eventual physical loss), and awaiting field
magnitudes. This is therefore a theoretical and synthetic measurement foundation,
not a claim of calibrated organizational ERC, FRR, or consequence curves. The
constructs are offered for
