...
\Cref{prop:steps} predicts the temporal signature of evidence-debt interest:
plateaus punctuated by cliffs, with smooth growth only where human memory is the
cheapest surviving source. This section makes the prediction concrete by computing
...
\begin{figure}[t]
\centering
\input{figs/fig-interest}
\caption{Model-derived interest trajectory for a fixed intent-link deficit under
the decay schedule of \cref{sec:interest-worked} (computed from the released
script; illustrative structure, not a field measurement). Expected reconstruction
cost (solid, left axis, log scale) is flat while the cheapest path (CI log)
...
saturates at the write-off cap as irrecoverability probability (dashed, right
axis) approaches one. The shape---plateaus, cliffs, hazard-driven slope,
saturation---is the general prediction of \cref{prop:steps}; none of it is
compound growth.}
\label{fig:interest}
\end{figure}

\Cref{fig:interest} plots the resulting expected cost and irrecoverability
...
Third, \emph{the saturation}: past the last non-human path, the deficit's cost is
dominated by the write-off penalty; further aging changes little because the loss
has effectively already occurred (\cref{prop:writeoff}).
...
queries is a step function of evidence age, with steps aligned to the estate's
