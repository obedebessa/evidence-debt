...
rather than by ongoing development friction; and it makes irrecoverability and
mandatory write-off an explicit first-class boundary in its formal model
(\cref{prop:writeoff})---no amount of later diligence recovers a record that no
...
The distinction is one of primary referent and granularity rather than universal
recoverability. Documentation debt primarily concerns missing or outdated
descriptive artifacts about system state and design; evidence debt concerns
time-indexed, event-level operational provenance and cross-store linkage needed to
reconstruct how the current state came into existence. Design rationale and other
documentation can also become irrecoverable. Their typical decay mechanics still
differ: documentation often rots by divergence from a living
system~\cite{parnas1994,lehman1980}, whereas operational evidence often rots by
destruction of sources and loss of joinability. The software-maintenance
...
perfect current documentation and no change history can have low documentation
debt while carrying high evidence debt under a demanding schema and workload.
...
system with zero configuration drift can carry high evidence debt about how
...
versus development friction), explicit terminal behavior (mandatory write-off
versus a first-class informational boundary beyond which the event cannot be
re-observed), different observability (invisible to system
inspection). \emph{Why not documentation debt?} Descriptive state/design artifacts
versus time-indexed event provenance and cross-store linkage
(\cref{sec:rel-doc}). \emph{Why
...
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{\input{figs/fig-tdvsed}}
...
shape, terminal evidence state, and policy-burden channels. The comparison is the condensed form
of \cref{sec:novelty}.}
\label{fig:tdvsed}
\end{figure}
