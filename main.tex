% =====================================================================
% Evidence Debt: Modeling the Deferred Reconstruction Burden of Missing
% Operational Evidence in Cloud-Native Systems
%
...
Evidence Debt: Modeling the Deferred Reconstruction\\[2pt]
Burden of Missing Operational Evidence in\\[2pt]
Cloud-Native Systems\par}
\vspace{14pt}
...
% ------------------------- abstract ----------------------------------
\noindent\textbf{\textit{Abstract}---We define \emph{evidence debt}: expected excess reconstruction burden created when infrastructure changes lack durable evidence of who acted, what changed, why, under which authorization, with what outcome, and how the system can be reconstructed. We contribute non-circular definitions; theoretical $\ED^{\star}$ and fixed-policy $\ED_\rho$ quantities; a decay-aware reconstruction model; analytic plateau, interaction, and bounded-loss results; a deterministic simulation; a bounded repository observation; and an unexecuted field protocol. Methodologically, $\rho$ decides whether to accept using evidence alone, \emph{before} ground truth scores an accepted answer; policy abstention is kept distinct from certified physical irrecoverability. Under this rule, 40\% degradation leaves 97.2\% supported coverage after intent-link stripping and 93.6\% after approval-link stripping, but 55.4\% after correlation-id stripping and 29.4\% after dense record deletion; accepted-wrong answers reach 30.3\% among accepted links in the latter condition. A paired path census observes complementary degradations destroying the last acceptable path. In 200 age-stratified Argo CD pull requests, 52.5\% lacked an external issue reference under a declared repository schema; review-record and release-lineage controls were 98.5\% and 100\%, and a seeded 50-PR audit matched collector labels. This observation establishes missing external linkage, not intent inadequacy, debt magnitude, decay, or reconstruction cost. The paper therefore establishes a theoretical and synthetic measurement foundation; field magnitudes remain open.}
