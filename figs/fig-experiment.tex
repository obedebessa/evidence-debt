...
\begin{tikzpicture}[node distance=3.6mm and 5mm]
  ...
  \node[brok, right=of gen, minimum width=24mm] (deg)
    {\textbf{degrader}\\{\smlbl intent $\cdot$ approval $\cdot$ corr-ids}\\[-1pt]{\smlbl records $\cdot$ timestamps $\cdot$ all}\\[-1pt]{\smlbl $p \in \{1,5,10,20,40\}\%$}};
  ...
  \node[lbl, below=1.5mm of deg, xshift=2mm, text=black!60]
    {20 seeds $\times$ 2 profiles $\times$ 6 classes $\times$ 6 levels};
\end{tikzpicture}
